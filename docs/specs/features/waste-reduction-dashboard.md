...
- 消費ログ（`consumption_logs`）が1件も無い（新規ユーザー等）場合はダイジェスト
  送信・ストリーク更新をスキップする（既存の `send-expiry-notifications` の
  「対象0件ならスキップ」方針と同様）。廃棄アイテムが0件のユーザーは対象外では
  なく、「廃棄ゼロを継続中」として通常通りダイジェスト送信・ストリーク評価を行う
  （#1044）

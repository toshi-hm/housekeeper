...
## 9. 拡張: レビュー画面での店舗別価格上昇アラート（#941）

### 課題

店舗別価格比較（`useStorePriceComparisons`、統計ページ、#697）は事後（購入後に
まとめて確認する）の導線のみで、レシート登録の確認画面（購入を確定する直前）で
価格変化に気づける導線が無い。

### スコープ

- やること: `ReceiptReviewPanel` / `ReceiptLineItemRow` の各行で、行の商品名（正規化
  一致、既存アイテム名との完全一致に限定し曖昧マッチはしない）× `storeName`
  （レビュー画面で確認・編集済みの店舗名）の組み合わせで `item_lots` から直近 N 件の
  `unit_price` 平均（または直近値）を取得し、一定割合（例: 10%以上）を超える上昇が
  あればバッジ + ツールチップで表示する。初回購入（比較対象データが無い、または
  商品名の完全一致が無い）行では何も表示しない（既存の店舗別価格比較カードと
  同じ「2件以上のデータがある場合のみ」という条件を踏襲）
- やらないこと: 登録をブロックしない（あくまで気づきの提供）。値上がり検知の
  しきい値をユーザー設定にする UI は v1 では作らない（固定値）。商品名の曖昧
  マッチ（表記揺れ吸収、#990 で別途検討）は本拡張のスコープ外

### データへの影響

なし。既存 `item_lots` データの読み取りのみ、新規の外部連携やAI呼び出しは追加しない。

### 技術ポイント

`useStoreNameSuggestions` 等の既存インフラを再利用し、レビュー画面表示時に対象行の
商品名（完全一致）× `storeName` で `item_lots` を検索するクエリを追加する
（`item_lots` の親 `items.name` との join、または `items` 側を先に名前一致検索して
`id` を得てから `item_lots` を引く2段クエリのいずれかで実装する）。

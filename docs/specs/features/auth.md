...
| `question`                  | text        | 選択時点のUI言語で翻訳された質問文言（`SECURITY_QUESTION_IDS` + `securityQuestionLabelKey`、`src/lib/auth.ts`）。DBには言語非依存のIDではなく文言そのものを保存する |
...
- 質問の選択肢（サインアップ画面のドロップダウン）は `t()` 経由でUIの言語設定に追従するようになった（#620）。ただし `question` カラムには選択時点の言語で翻訳済みの文言をそのまま保存するため、登録後に言語設定を変えてもリセットフローで表示される質問文言は登録時の言語のまま残る。言語非依存のID保存への移行は既存データのマイグレーションを伴うため別途の対応とする

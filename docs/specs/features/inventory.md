...
- ダッシュボード（`/_auth/`）: 判定に該当するアイテムを検索・カテゴリ・場所・期限の一覧フィルター
  にかかわらず対象として、低在庫バナー（件数・開閉可能な内訳・「まとめて買い物リストに追加」
  ボタン）を表示する。ただし「在庫0を隠す」トグル（`hideEmpty`）はこのバナーにも適用される
  （`lowStockItems` は `baseFiltered` から算出されるため）。期限切れ／期限間近バナー
  （`urgentItems`、`items` から直接算出）はこのトグルの影響を受けない点で挙動が異なる。
...
過去に登録した同一商品があれば保管場所セレクトを自動で事前選択し、「買い足し」のような
頻出ケースでの手動選択の手間を減らす。

- `useSuggestedLocation`（`src/hooks/useSuggestedLocation.ts`）: バーコード完全一致 → 無ければ
  商品名の完全一致（大文字小文字無視）の順で、ログインユーザーの `items`（ソフトデリート除く）
  から直近更新（`updated_at` 降順）の `storage_location_id` を1件取得する。新規テーブルは
  持たず、`useBarcodeLookup` の DB優先ルックアップと同じ `items` テーブルを問い合わせる。
- 適用条件: バーコードスキャン成功時、または商品名フィールドからのフォーカスアウト時に照会し、
  ユーザーがまだ保管場所を一度も選択（クイック追加含む）していない間だけ、サジェストされた値を
  保管場所セレクトの表示・送信値として使う（実際の `items.storage_location_id` は未確定のまま
  保持し、setState をeffect内から呼ばない派生値として扱う）。ユーザーが一度でも手動選択したら、
  以降そのセッション中はサジェストを提案しない。
- UI: サジェストが適用されている間、セレクト下に小さく案内文言（`suggestedLocationHint`）を
  表示し、ユーザーの意図的な選択と誤認しないようにする。
- 既存アイテムの編集画面（`EditItemPage`）では `enableLocationSuggestion` を渡さないため対象外
  （実データが既に入っているため）。

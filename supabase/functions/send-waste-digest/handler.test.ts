...
Deno.test("send-waste-digest handler - responds to preflight", async () => {
  const response = await handler(new Request("https://example.test", { method: "OPTIONS" }));
  assert.strictEqual(response.status, 200);
});

Deno.test("send-waste-digest handler - rejects requests without a valid cron secret", async () => {
...
Deno.test("send-waste-digest handler - rejects requests when CRON_SECRET is not configured", async () => {
  const previous = Deno.env.get("CRON_SECRET");

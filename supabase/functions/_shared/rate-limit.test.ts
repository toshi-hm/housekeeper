...
// checkChatRateLimit / checkRecipeRateLimit (supabase.rpc(fn, params).single()),
// so the RPC boundary itself can be exercised without a live database.
...
Deno.test("checkRecipeRateLimit - returns allowed when the RPC reports under the limit", async () => {
  const supabase = makeFakeClient({ data: { allowed: true, retry_after_seconds: 0 }, error: null });
  const result = await checkRecipeRateLimit(supabase);
  assert.deepStrictEqual(result, { allowed: true, retryAfterSeconds: 0 });
});

Deno.test("checkRecipeRateLimit - returns blocked with retry-after when over the limit", async () => {
  const supabase = makeFakeClient({
    data: { allowed: false, retry_after_seconds: 42 },
    error: null,
  });
...
Deno.test("checkRecipeRateLimit - fails closed when the RPC returns an error", async () => {
  const supabase = makeFakeClient({ data: null, error: new Error("boom") });
  const result = await checkRecipeRateLimit(supabase);
  assert.strictEqual(result.allowed, false);
  assert.strictEqual(result.retryAfterSeconds, 60);
...
Deno.test("checkRecipeRateLimit - fails closed when the RPC returns no data", async () => {
  const supabase = makeFakeClient({ data: null, error: null });
  const result = await checkRecipeRateLimit(supabase);
  assert.strictEqual(result.allowed, false);
  assert.strictEqual(result.retryAfterSeconds, 60);
...
Deno.test("checkRecipeRateLimit - calls the server-configured RPC without caller-controlled limits", async () => {
  let capturedFn: string | undefined;
  let capturedParams: Record<string, unknown> | undefined;
  const supabase = makeFakeClient(
    { data: { allowed: true, retry_after_seconds: 0 }, error: null },

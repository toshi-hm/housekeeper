-- #747: import_items_batch (20260802000001_add_store_name_to_import_items_batch.sql)
-- inserted `v_lot ->> 'store_name'` verbatim, unlike the normal write path
-- (createLot/updateLot in src/hooks/useItemLots.ts) which trims the value and
-- coerces an empty string to NULL via normalizeStoreName. A hand-edited or
-- older-format import JSON with `"store_name": ""` (or whitespace-only) would
-- persist an empty string, which then leaks into the store-name suggestion
-- datalist (fetchStoreNameSuggestions filters only `is not null`) while being
-- silently excluded from computeStorePriceComparisons (which treats a falsy
-- store_name as absent). Normalize the same way on import: trim + empty -> NULL.

...
-- once p_window_seconds has elapsed since window_start, the count resets.
-- Returns allowed = false once more than p_max_requests requests have been
...
revoke all on function public.check_chat_rate_limit(int, int) from public;
grant execute on function public.check_chat_rate_limit(int, int) to authenticated;

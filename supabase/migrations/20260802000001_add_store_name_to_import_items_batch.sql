-- #697: `import_items_batch` (20260731000001_atomic_import_items.sql) hard-codes
-- the item_lots columns it reads from each lot's JSON, so the newly added
-- `item_lots.store_name` column was silently dropped on import even though
-- the JSON export/import schema now carries it (src/lib/export.ts). Replace
-- the function to also read/write `store_name`, keeping every other behavior
-- identical (same signature, same duplicate-strategy branches).
--
-- p_items shape: unchanged except each lot object may now also carry

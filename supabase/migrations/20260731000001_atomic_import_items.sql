-- #694: importItems previously ran as a client-side loop of separate Supabase
-- calls (delete lots -> insert item/lots -> update item) with no transaction.
-- A failure partway through left the already-processed items committed and
-- the rest untouched, and the caller had no way to tell how far it got.
-- Retrying the same file then re-created barcode-less items that had already
-- succeeded, since duplicate detection only keys off barcode.
--
-- Move the whole batch into a single Postgres function so it runs as one
-- atomic transaction: either every item in the batch is applied, or (on any
-- error) none of it is — so a retry after a failure can never see partial
-- state to duplicate. Follows the same pattern as
-- save_shopping_list_template (#573, 20260724000001_atomic_save_shopping_template.sql).
--
-- p_items shape (one entry per ImportItemInput, `lots` per ImportLotInput):
-- [{
--   "name": text, "barcode": text|null, "content_amount": numeric,
--   "content_unit": text, "notes": text|null, "minimum_stock": int|null,
--   "auto_reorder": boolean|null, "reorder_threshold": int|null,
--   "lots": [{ "units": int, "opened_remaining": numeric|null,
--              "unit_price": int|null, "purchase_date": date|null,
...
begin
  ...
  loop
    ...
    if v_existing_id is not null and p_duplicate_strategy = 'overwrite' then
      ...
      loop
        insert into item_lots (user_id, item_id, units, opened_remaining, unit_price, purchase_date, expiry_date)
        values (
          auth.uid(),
          v_existing_id,
          (v_lot ->> 'units')::int,
    ...
    insert into items (
      user_id, name, barcode, content_amount, content_unit, notes,

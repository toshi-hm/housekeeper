...
#### 1) 基盤 / 共通
- [ ] `bun test` 環境構築（happy-dom + bun:test の preload 設定）
- [ ] ESLint / oxlint / oxfmt をフックで CI 化（`bun run check` を追加）
- [ ] エラーハンドリング標準化（Toast 共通、ErrorBoundary、4 状態）
- [ ] 削除アクションに `ConfirmDialog` を導入
- [ ] i18n 基盤導入（react-i18next + ja/en, 名前空間: `common`/`items`/`auth`/`settings`/`shopping`/`stats`）
- [ ] `user_settings` テーブル新設 + 言語 / 期限閾値 / デフォルト単位 / 通知時刻

#### 2) データモデル / DB
- [ ] migration 追加: `categories`
- [ ] migration 追加: `storage_locations`
- [ ] migration 追加: items 拡張（`units`, `content_amount`, `content_unit`, `opened_remaining`, `category_id`, `storage_location_id`）
- [ ] migration 追加: `consumption_logs`
- [ ] migration 追加: `user_settings`
- [ ] migration 追加: `item-images` Storage バケット + RLS
- [ ] 既存 `items.quantity` のデータマイグレーション戦略（→ `units` に移行、`content_amount=1`、`content_unit='個'` を初期値）

#### 3) 在庫機能
- [ ] `useItems` を新スキーマに対応（filter / sort / pagination）
- [ ] `ItemForm` 改修（QuantityInput / UnitToggle / ImageUploader）
- [ ] `ItemConsumePage` 新規（消費フォーム + アルゴリズム実装）
- [ ] `useConsumeItem` hook 追加（楽観更新）
- [ ] 一覧フィルタ UI（カテゴリ / 場所 / 期限ステータス）
- [ ] 一覧ソート UI（期限近い順 / 購入日 / 作成日）
- [ ] 「使い切り」の淡色化 + 既定で隠す

#### 4) マスタ管理
- [ ] `useCategories` / `useStorageLocations` hook
- [ ] `MasterDataPage`（カテゴリ）
- [ ] `MasterDataPage`（保管場所）
- [ ] ItemForm の category / location を Select に変更

#### 5) 画像
- [ ] `useUploadItemImage` hook（Supabase Storage）
- [ ] `ImageUploader` molecule（drop / preview / 差し替え）
- [ ] `ItemImage` atom（signed URL の自動更新）

#### 6) バーコード
- [ ] `BarcodeScanner` の読取失敗時のリトライ UI
- [ ] スキャン後の手入力フォールバック導線
- [ ] `barcode-lookup` の戻り値拡張（カテゴリのマッピング案を入れる）

#### 7) 期限切れ
- [ ] `EXPIRY_WARNING_DAYS` 定数を `user_settings.expiry_warning_days` に置換
- [ ] `ExpiryBadge` のテストを `bun test` で追加

#### 8) i18n / 設定
- [ ] `LanguageToggle` atom
- [ ] `SettingsPage`（言語 / 閾値 / デフォルト単位）
- [ ] 全画面の文言外出し（ja.json / en.json）

#### 9) テスト / VRT
- [ ] `getExpiryStatus` の境界値テスト
- [ ] 消費アルゴリズムのテスト（`opened_remaining` 跨ぎ、units=0、丸め誤差）
- [ ] 主要 hook の happy path テスト
- [ ] 新 organisms の Storybook + VRT 登録

### v1.1 — 買い物リスト / 履歴

- [ ] migration: `shopping_list_items`
- [ ] `useShoppingList` / `useUpsertShoppingItem` / `usePurchaseShoppingItem`
- [ ] `ShoppingPage` + `ShoppingList` organism
- [ ] item 詳細に「補充」ボタン → `shopping_list_items` 追加
- [ ] 購入完了モーダル → `items` 行作成
- [ ] item 詳細に「履歴」タブ（consumption_logs）

### v1.2 — PWA / 通知

- [ ] `vite-plugin-pwa` 導入（manifest / icons / install prompt）
- [ ] `@tanstack/query-sync-storage-persister` + IndexedDB
- [ ] サービスワーカー: stale-while-revalidate（一覧 / 詳細 / 画像）
- [ ] migration: `notification_preferences`, `push_subscriptions`
- [ ] Edge Function `send-expiry-notifications`
- [ ] Edge Function `subscribe-push`（VAPID 鍵管理）
- [ ] `NotificationSettings` organism（Push / Email / 閾値 / 時刻）
- [ ] Resend など Email プロバイダ選定 + 環境変数

### v1.3 — 統計

- [ ] Recharts 導入
- [ ] `useStats` hook（集計クエリ群）
- [ ] `StatsPage` + 3 グラフ organism

...
- クライアントの `console.error` に加え、`VITE_SENTRY_DSN` を設定した場合のみ Sentry へエラー送信（opt-in、既定は無効）
  <!-- issue:#67 --> 詳細: `docs/specs/architecture.md` Observability セクション
...
> - 1 行 = 1 Issue を原則
> - Skill `issue-sync`（`.claude/skills/project/issue-sync/SKILL.md`）で未起票分を自動起票・チェック済みを close
...
- [x] DB migration: `items.deleted_at timestamptz null` 追加（ソフトデリート） <!-- issue:#83 -->
- [x] `useItems` を `deleted_at IS NULL` フィルタ対応 + `itemSchema` 更新 <!-- issue:#84 -->
- [x] `useSoftDeleteItem` hook 追加（チェックボックスで論理削除） <!-- issue:#85 -->
- [x] `useCreateItem` にバーコード一致ソフトデリート品の復活ロジック追加 <!-- issue:#86 -->
- [x] カテゴリ管理にカラーピッカー追加（`categories.color` 活用） <!-- issue:#87 -->
- [x] フッター 5 メニュー化（Calendar 追加、Add Item を `justify-around` で常に中央） <!-- issue:#88 -->
- [x] `_auth.calendar.tsx` ルート + 上部サマリーセクション（期限切れ赤/今週黄/今月その他） <!-- issue:#89 -->
- [x] チェックボックス → 斜線 UI + `useSoftDeleteItem` 呼び出し <!-- issue:#90 -->
- [x] `ExpiryCalendar` organism（月ビュー、カテゴリ別色分けイベント） <!-- issue:#91 -->

### v1.5 — 在庫 AI チャット（Gemini 無料枠）
...
- [x] 定期購入予測 / 在庫推移グラフ（消費ペース予測・消費速度ランキングとして実装。#392 と統合） <!-- issue:#68 -->
...
- `.claude/skills/project/issue-sync/SKILL.md`（Issue 同期 Skill）

...
#### 1) 基盤 / 共通
- [ ] `bun test` 環境構築（happy-dom + bun:test の preload 設定） <!-- issue:#8 -->
- [ ] ESLint / oxlint / oxfmt をフックで CI 化（`bun run check` を追加） <!-- issue:#9 -->
- [ ] エラーハンドリング標準化（Toast 共通、ErrorBoundary、4 状態） <!-- issue:#10 -->
- [ ] 削除アクションに `ConfirmDialog` を導入 <!-- issue:#11 -->
- [ ] i18n 基盤導入（react-i18next + ja/en, 名前空間: `common`/`items`/`auth`/`settings`/`shopping`/`stats`） <!-- issue:#12 -->
- [ ] `user_settings` テーブル新設 + 言語 / 期限閾値 / デフォルト単位 / 通知時刻 <!-- issue:#13 -->

#### 2) データモデル / DB
- [ ] migration 追加: `categories` <!-- issue:#14 -->
- [ ] migration 追加: `storage_locations` <!-- issue:#15 -->
- [ ] migration 追加: items 拡張（`units`, `content_amount`, `content_unit`, `opened_remaining`, `category_id`, `storage_location_id`） <!-- issue:#16 -->
- [ ] migration 追加: `consumption_logs` <!-- issue:#17 -->
- [ ] migration 追加: `user_settings` <!-- issue:#18 -->
- [ ] migration 追加: `item-images` Storage バケット + RLS <!-- issue:#19 -->
- [ ] 既存 `items.quantity` のデータマイグレーション戦略（→ `units` に移行、`content_amount=1`、`content_unit='個'` を初期値） <!-- issue:#20 -->

#### 3) 在庫機能
- [ ] `useItems` を新スキーマに対応（filter / sort / pagination） <!-- issue:#21 -->
- [ ] `ItemForm` 改修（QuantityInput / UnitToggle / ImageUploader） <!-- issue:#22 -->
- [ ] `ItemConsumePage` 新規（消費フォーム + アルゴリズム実装） <!-- issue:#23 -->
- [ ] `useConsumeItem` hook 追加（楽観更新） <!-- issue:#24 -->
- [ ] 一覧フィルタ UI（カテゴリ / 場所 / 期限ステータス） <!-- issue:#25 -->
- [ ] 一覧ソート UI（期限近い順 / 購入日 / 作成日） <!-- issue:#26 -->
- [ ] 「使い切り」の淡色化 + 既定で隠す <!-- issue:#27 -->

#### 4) マスタ管理
- [ ] `useCategories` / `useStorageLocations` hook <!-- issue:#28 -->
- [ ] `MasterDataPage`（カテゴリ） <!-- issue:#29 -->
- [ ] `MasterDataPage`（保管場所） <!-- issue:#30 -->
- [ ] ItemForm の category / location を Select に変更 <!-- issue:#31 -->

#### 5) 画像
- [ ] `useUploadItemImage` hook（Supabase Storage） <!-- issue:#32 -->
- [ ] `ImageUploader` molecule（drop / preview / 差し替え） <!-- issue:#33 -->
- [ ] `ItemImage` atom（signed URL の自動更新） <!-- issue:#34 -->

#### 6) バーコード
- [ ] `BarcodeScanner` の読取失敗時のリトライ UI <!-- issue:#35 -->
- [ ] スキャン後の手入力フォールバック導線 <!-- issue:#36 -->
- [ ] `barcode-lookup` の戻り値拡張（カテゴリのマッピング案を入れる） <!-- issue:#37 -->

#### 7) 期限切れ
- [ ] `EXPIRY_WARNING_DAYS` 定数を `user_settings.expiry_warning_days` に置換 <!-- issue:#38 -->
- [ ] `ExpiryBadge` のテストを `bun test` で追加 <!-- issue:#39 -->

#### 8) i18n / 設定
- [ ] `LanguageToggle` atom <!-- issue:#40 -->
- [ ] `SettingsPage`（言語 / 閾値 / デフォルト単位） <!-- issue:#41 -->
- [ ] 全画面の文言外出し（ja.json / en.json） <!-- issue:#42 -->

#### 9) テスト / VRT
- [ ] `getExpiryStatus` の境界値テスト <!-- issue:#43 -->
- [ ] 消費アルゴリズムのテスト（`opened_remaining` 跨ぎ、units=0、丸め誤差） <!-- issue:#44 -->
- [ ] 主要 hook の happy path テスト <!-- issue:#45 -->
- [ ] 新 organisms の Storybook + VRT 登録 <!-- issue:#46 -->

### v1.1 — 買い物リスト / 履歴

- [ ] migration: `shopping_list_items` <!-- issue:#47 -->
- [ ] `useShoppingList` / `useUpsertShoppingItem` / `usePurchaseShoppingItem` <!-- issue:#48 -->
- [ ] `ShoppingPage` + `ShoppingList` organism <!-- issue:#49 -->
- [ ] item 詳細に「補充」ボタン → `shopping_list_items` 追加 <!-- issue:#50 -->
- [ ] 購入完了モーダル → `items` 行作成 <!-- issue:#51 -->
- [ ] item 詳細に「履歴」タブ（consumption_logs） <!-- issue:#52 -->

### v1.2 — PWA / 通知

- [ ] `vite-plugin-pwa` 導入（manifest / icons / install prompt） <!-- issue:#53 -->
- [ ] `@tanstack/query-sync-storage-persister` + IndexedDB <!-- issue:#54 -->
- [ ] サービスワーカー: stale-while-revalidate（一覧 / 詳細 / 画像） <!-- issue:#55 -->
- [ ] migration: `notification_preferences`, `push_subscriptions` <!-- issue:#56 -->
- [ ] Edge Function `send-expiry-notifications` <!-- issue:#57 -->
- [ ] Edge Function `subscribe-push`（VAPID 鍵管理） <!-- issue:#58 -->
- [ ] `NotificationSettings` organism（Push / Email / 閾値 / 時刻） <!-- issue:#59 -->
- [ ] Resend など Email プロバイダ選定 + 環境変数 <!-- issue:#60 -->

### v1.3 — 統計

- [ ] Recharts 導入 <!-- issue:#61 -->
- [ ] `useStats` hook（集計クエリ群） <!-- issue:#62 -->
- [ ] `StatsPage` + 3 グラフ organism <!-- issue:#63 -->

...
describe("computeConsumptionPaceForecast", () => {
  ...
  test("no logs → insufficient data (null rate/days, logCount 0)", () => {
    const result = computeConsumptionPaceForecast([], 10, 30, now);
  ...
  test("currentStock <= 0 → predictedRemainingDays is 0 regardless of logs", () => {
    const logs = [{ delta_amount: 5, occurred_at: "2026-04-25T00:00:00Z" }];
    const result = computeConsumptionPaceForecast(logs, 0, 30, now);
  ...
  test("fewer than 2 logs in the window → insufficient data but logCount is reported", () => {
    const logs = [{ delta_amount: 5, occurred_at: "2026-04-25T00:00:00Z" }];
    const result = computeConsumptionPaceForecast(logs, 10, 30, now);
    expect(result.dailyRate).toBeNull();
  ...
  test("computes dailyRate and floors predictedRemainingDays", () => {
    const logs = [
      { delta_amount: 10, occurred_at: "2026-04-01T00:00:00Z" },
      { delta_amount: 10, occurred_at: "2026-04-10T00:00:00Z" },
      { delta_amount: 10, occurred_at: "2026-04-20T00:00:00Z" },
    ];
    // total 30 over a 30-day lookback → 1/day; stock 15 → 15 days remaining
    const result = computeConsumptionPaceForecast(logs, 15, 30, now);
  ...
  test("excludes logs outside the lookback window", () => {
    const logs = [
      { delta_amount: 1000, occurred_at: "2026-01-01T00:00:00Z" }, // way before cutoff
      { delta_amount: 10, occurred_at: "2026-04-05T00:00:00Z" },
      { delta_amount: 10, occurred_at: "2026-04-15T00:00:00Z" },
    ];
    const result = computeConsumptionPaceForecast(logs, 100, 30, now);
  ...
  test("excludes logs from the future", () => {
    const logs = [
      { delta_amount: 10, occurred_at: "2026-04-10T00:00:00Z" },
      { delta_amount: 10, occurred_at: "2026-05-15T00:00:00Z" }, // after `now`
    ];
    const result = computeConsumptionPaceForecast(logs, 100, 30, now);
    expect(result.logCount).toBe(1);
  });
...
describe("computeConsumptionSpeedRanking", () => {
  ...
  const logs: ItemConsumptionLogEntry[] = [
    ...
    { item_id: "item-a", delta_amount: 10, occurred_at: "2026-04-22T00:00:00Z" },
    { item_id: "item-a", delta_amount: 10, occurred_at: "2026-04-26T00:00:00Z" },
    { item_id: "item-a", delta_amount: 5, occurred_at: "2026-04-12T00:00:00Z" },
    // item-b: recent 5 (decelerating vs prior 20)
    { item_id: "item-b", delta_amount: 5, occurred_at: "2026-04-24T00:00:00Z" },
    { item_id: "item-b", delta_amount: 20, occurred_at: "2026-04-13T00:00:00Z" },
    // item-c: recent 10 (steady vs prior 10)
    { item_id: "item-c", delta_amount: 10, occurred_at: "2026-04-23T00:00:00Z" },
    { item_id: "item-c", delta_amount: 10, occurred_at: "2026-04-14T00:00:00Z" },
    // item-d: recent 3, no prior logs → insufficient-data
    { item_id: "item-d", delta_amount: 3, occurred_at: "2026-04-21T00:00:00Z" },
    // item-e: only logs older than 2*windowDays → should not appear at all
    { item_id: "item-e", delta_amount: 999, occurred_at: "2026-03-01T00:00:00Z" },
    // item-f: only a future log → should not appear at all
    { item_id: "item-f", delta_amount: 999, occurred_at: "2026-05-15T00:00:00Z" },
  ];

  test("returns [] for no logs", () => {
    expect(computeConsumptionSpeedRanking([], windowDays, now)).toEqual([]);
  });

  test("sorts descending by recent daily rate", () => {
    const result = computeConsumptionSpeedRanking(logs, windowDays, now);
...
describe("computeForecastAlerts", () => {
  ...
  test("skips items that are already out of stock", () => {
    const items = [{ id: "empty", units: 0, content_amount: 1, opened_remaining: null }];
    const logs: ItemConsumptionLogEntry[] = [
      { item_id: "empty", delta_amount: 10, occurred_at: "2026-04-25T00:00:00Z" },
    ];
    expect(computeForecastAlerts(items, logs, 7, lookbackDays, now)).toEqual([]);
  });

  test("excludes items with insufficient consumption data", () => {
    const items = [{ id: "no-history", units: 10, content_amount: 1, opened_remaining: null }];
  ...
  test("excludes items whose predicted remaining days exceed the threshold", () => {
    ...
    const items = [{ id: "slow-mover", units: 3, content_amount: 2, opened_remaining: null }];
    const logs: ItemConsumptionLogEntry[] = [
      { item_id: "slow-mover", delta_amount: 1, occurred_at: "2026-04-24T00:00:00Z" },
      { item_id: "slow-mover", delta_amount: 1, occurred_at: "2026-04-28T00:00:00Z" },
    ];
    expect(computeForecastAlerts(items, logs, 7, lookbackDays, now)).toEqual([]);
  });
  ...
  test("includes and sorts items within the threshold by urgency (fewest days first)", () => {
    const items = [
      { id: "five-days", units: 5, content_amount: 1, opened_remaining: null },
      { id: "two-days", units: 2, content_amount: 1, opened_remaining: null },
    ];
    const logs: ItemConsumptionLogEntry[] = [
      // five-days: stock 5, total 10 over 10 days → 1/day → 5 days remaining
      { item_id: "five-days", delta_amount: 5, occurred_at: "2026-04-24T00:00:00Z" },
      { item_id: "five-days", delta_amount: 5, occurred_at: "2026-04-28T00:00:00Z" },
      // two-days: stock 2, total 8 over 10 days → 0.8/day → floor(2/0.8) = 2 days remaining
      { item_id: "two-days", delta_amount: 4, occurred_at: "2026-04-24T00:00:00Z" },
      { item_id: "two-days", delta_amount: 4, occurred_at: "2026-04-28T00:00:00Z" },
    ];

 ...
 *
 * `item_lots` は商品名を持たないため、`items` 側を先に取得（`useItemsForExport`
 * を再利用）して `id -> name` の対応表を作り、ロット行と突き合わせる
 * 2段クエリで実装する（同spec「技術ポイント」節の選択肢のうち後者）。

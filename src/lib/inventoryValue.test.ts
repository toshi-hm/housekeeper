...
describe("computeInventoryValue", () => {
  test("returns null for empty lots", () => {
    expect(computeInventoryValue([])).toBeNull();
  });
  ...
  test("returns null when unit_price is undefined", () => {
    const result = computeInventoryValue([{ units: 2, unit_price: undefined }]);
    expect(result).toBeNull();
  });

  test("single lot: units × unit_price", () => {
    const result = computeInventoryValue([{ units: 3, unit_price: 150 }]);
  ...
  test("unit_price = 0 is treated as a valid, priced value (free item)", () => {
    const result = computeInventoryValue([{ units: 4, unit_price: 0 }]);
